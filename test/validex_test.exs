defmodule ValidexTest do
  ...
  test "can mix shorthand and fullform when one is absent" do
    assert [{:ok, :age, :presence}, {:ok, :age, :type}, {:error, :name, :presence, _}] =
  ...
  test "unexpected properties are errors when using strict mode" do
    assert [{:error, :hat, :strict, "hat is an unexpected attribute"}] =
             Validex.errors(%{hat: "nisse", name: "Simon"}, [name: :string], strict: true)

    assert [{:error, [:hat, :kat], :strict, "kat is an unexpected attribute"}] =
             Validex.errors(
               %{hat: %{kat: "nisse", name: "Simon"}},
               [hat: %{name: :string}],
               strict: true
             )

    assert [_, {:error, [:hat, :kat], :strict, "kat is an unexpected attribute"}] =
             Validex.errors(
               %{hat: %{kat: "nisse", name: "Simon"}},
               [hat: [one_of: [%{name: :string}]]],
               strict: true
             )
    ...
    assert [] =
             Validex.errors(
               %{hat: %{kat: "nisse", name: "Simon"}},
               [hat: [one_of: [%{name: :string}, %{name: :string, kat: :string}]]],
               strict: true
             )
  end
  ...
  test "unknown validators are errors" do
    assert [
             {:error, :name, :__validex__unknown_validator__,
              "name has unknown validator wtfbbq with spec :roflcopter"}
